...
<body>
   ...
   ?>
   <script>
          $('head').append('<link rel="stylesheet" href="../farmer/css/myposts.css" type="text/css" />');
   </script>
   <div class="col mt-5">
   <?php
    $pdt = $_GET['pdt'];
            ...
            if($result) {
                ...
                if(mysqli_num_rows($result) > 0) {
                    ...
                    while ($row = mysqli_fetch_row($result)) {
                       // if(($row[8] > date("Y-m-d") and $sel=='fruitsandveges') or $sel=='machines' or $sel=='seeds') {
                        if ($sel == 'machines') {
...
?>
                    <div class="post-card border">
                        <div class="meta">
                            <?php
                           echo '<img class="photo" src="../farmer/img/'.$sel.'/'.$row[10].'" alt="img">';
                           $img = $row[10];
